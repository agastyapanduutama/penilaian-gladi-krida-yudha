...
class Penilaiankelompok extends CI_Controller
{
    ...
    public function Tambah($id = null)
    {
        try {
            ...
            // nilai produk
            $NilaiProduk = $this->pkelompok->NilaiProduk($id);
            $data['NilaiProduk'] = 0;
            if ($NilaiProduk > 0)
                $data['NilaiProduk'] = $NilaiProduk;

            // nilai posko
            $NilaiPosko = $this->pkelompok->NilaiPosko($id);
            $data['NilaiPosko'] = 0;
            if ($NilaiPosko > 0)
                $data['NilaiPosko'] = $NilaiPosko;

            // nilai telegram
            $NilaiTelegram = $this->pkelompok->NilaiTelegram($id);
            $data['NilaiTelegram'] = 0;
            if ($NilaiTelegram > 0)
                $data['NilaiTelegram'] = $NilaiTelegram;

            // nilai telepon
            $NilaiTelepon = $this->pkelompok->NilaiTelepon($id);
            $data['NilaiTelepon'] = 0;
            if ($NilaiTelepon > 0)
                $data['NilaiTelepon'] = $NilaiTelepon;

            $this->load->model('ModelPenilaianPerorangan', 'pperorangan');
            $data['TotalNilai'] = $this->pperorangan->TotalNilai($id);
            $this->load->library('Library');
    ...
    public function index()
    {
        ...
        try {
            ...
            $this->load->view('PenilaianKelompok/Data', $data);
        } catch (Exception $Error) {
            $this->session->set_flashdata('pesan', "<script>pesan_warning('Pesan','" . ($Error->getMessage()) . "')</script>");
            redirect();
        } catch (Throwable $Error) {
