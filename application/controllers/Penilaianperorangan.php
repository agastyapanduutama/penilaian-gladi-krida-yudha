...
class Penilaianperorangan extends CI_Controller
{
    ...
    public function index($IdKelompok = null)
    {
        try {
            ...
            $ReadAnggota = $this->pperorangan->CekAnggota($IdKelompok);

            $ReadKelompok = $this->pperorangan->ReadKelompok($IdKelompok);
            if ($ReadKelompok['count'] <= 0)
                throw new Exception('Kelompok tidak ditemukan');

            // info ReadKelompokInfo
            $this->load->model('ModelPenilaianKelompok', 'pkelompok');
            $GetInfoKelompok = $this->pkelompok->ReadKelompokInfo($IdKelompok);
            ...
            $data['TotalNilai'] = $this->pperorangan->TotalNilai($IdKelompok);
            // $data['nama_jabatan'] = "as";
